...
export default function AccountsAuth() {
  ...
  useEffect(() => {
    if (!authLoading && profile?.role === 'accounts') {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (mode === 'login') {
        ...
        const { token, user } = await api.post('/auth/signup', { name, email, password, role: 'accounts' })
        login(token, user)
      }
      navigate('/dashboard/accounts')
    } catch (err: any) {
    ...
    >
      <div className="w-full max-w-[400px] bg-bg-2 border border-white/[0.06] rounded-r-3 p-8 animate-fade-up">
        ...
        <p className="text-text-3 text-sm mb-6">
          {mode === 'login' ? 'Sign in to Accounts Head portal' : 'Set up your Accounts Head access'}
        </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <input
            type="email"
            placeholder={mode === 'login' ? 'rishabh@agency.com' : 'Email'}
            value={email}
