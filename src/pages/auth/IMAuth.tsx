...
export default function IMAuth() {
  ...
  useEffect(() => {
    if (!authLoading && profile?.role === 'im') {
      navigate('/dashboard/im')
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (mode === 'login') {
        ...
        const { token, user } = await api.post('/auth/signup', { name: name.trim(), email, password, role: 'im', im_member_name: name.trim() })
        login(token, user)
      }
      navigate('/dashboard/im')
    } catch (err: any) {
    ...
    >
      <div className="w-full max-w-[400px] bg-bg-2 border border-white/[0.06] rounded-r-3 p-8 animate-fade-up">
        ...
        <p className="text-text-3 text-sm mb-6">
          {mode === 'login' ? 'Sign in to your IM account' : 'Create your IM account'}
        </p>
